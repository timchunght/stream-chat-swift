...
class RequestRecorderURLProtocol: URLProtocol {
    ...
    ///
    /// - Parameter timeout: Specifies the time the function waits for a new request
    /// to be made.
    static func getLatestRequestSynchronously(timeout: TimeInterval) -> URLRequest? {
        defer {
        ...
        _ = XCTWaiter.wait(for: [latestRequestExpectation!], timeout: 1)
